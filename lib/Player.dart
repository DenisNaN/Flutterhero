...
import 'package:flutterhero/GameObject.dart';
...
class Player extends GameObject{
  ...
  /// Constructor.
  Player(double inScreenWidth, double inScreenHeight, String inBaseFilename, int inWidth, int inHeight,
      int inNumFrames, int inFrameSkip, int inSpeed, Function inFunction
      ) : super(inScreenWidth, inScreenHeight, inBaseFilename, inWidth, inHeight, inNumFrames, inFrameSkip, inFunction) {
    speed = inSpeed;
  }
  ...
  @override
  Widget draw(){
    return visible ? Positioned(left: x, top: y, child: Transform.rotate(angle: radians, child: frames[currentFrame]))
  ...
  void orientationChanged(){
    radians = 0.0;
    if(moveHorizontal == 1 && moveVertical == -1) radians = anglesToRadiansConversionTable["angle45"];
    if(moveHorizontal == 1 && moveVertical == 0) radians = anglesToRadiansConversionTable["angle90"];
    if(moveHorizontal == 1 && moveVertical == 1) radians = anglesToRadiansConversionTable["angle135"];
    if(moveHorizontal == 0 && moveVertical == 1) radians = anglesToRadiansConversionTable["angle180"];
    if(moveHorizontal == -1 && moveVertical == 1) radians = anglesToRadiansConversionTable["angle225"];
    if(moveHorizontal == -1 && moveVertical == 0) radians = anglesToRadiansConversionTable["angle270"];
    if(moveHorizontal == -1 && moveVertical == -1) radians = anglesToRadiansConversionTable["angle315"];
  }
