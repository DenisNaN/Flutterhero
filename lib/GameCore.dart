...
/// Audioplayer.  Needed in order to be able to play audio from assets.
...
void firstTimeInitilization(BuildContext inContext, dynamic inState){
...
void resetGame(bool inResetEnimies){
...
void gameLoop(){
  ...
  for(int i = 0; i < 3; i++){
    if(collision(fish[i]) || collision(robots[i]) || collision(aliens[i]) || collision(asteroids[i])){
      audioplayer.play(AssetSource('explosion.mp3'));
      player.visible = false;
      GameObject explosion = GameObject(screenWidth, screenHeight, "explosion", 50, 50, 5, 4, (){resetGame(false);});
...
void transferEnergy(bool inTouchingCrystal){
  ...
  if(inTouchingCrystal && player.energy < 1){
    ...
    if(player.energy == 0)
      audioplayer.play(AssetSource('fill.mp3'));
    ...
    if(player.energy > 0)
      audioplayer.play(AssetSource('delivery.mp3'));
    ...
    if(player.energy <= 0){
      ...
      for(int i = 0; i < 3; i++){
        ...
        explosion = GameObject(screenWidth, screenHeight, "explosion", 50, 50, 5, 4, callback);
